...
function removeClass( classname, elementId ) {
    ...
    element.className = cn;
	      ...
	      removeClass('selected', "li_" + prevViewedTab.getAttribute("id"));
            }

            var tabToDisplay = document.getElementById(idOfTabToDisplay);
              tabToDisplay.style.display = 'block';
              addClass('selected', "li_" + tabToDisplay.getAttribute("id"));
	...
	nodes = arrName;
		...
		if (startNode != 0) {
			var nodeValues = nodes[getArrayId(startNode)].split("|");
                        ...
                        "<img src=\"img/folderopen.gif\" align=\"absbottom\" alt=\"\"/>" + nodeValues[2] + "</a><br/>");
		}
		else {
		...
		if (nodeValues[1] == parentNode) {
			...
			if (hcn) {
				if (ls) {
					...
					document.write("bottom.gif\" align=\"absbottom\" alt=\"Open/Closenode\" /></a>");
				} else {
